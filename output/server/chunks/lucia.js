...
import { p as prisma$1 } from "./db.js";
...
const googleAuth = (() => {
  if (process.env.GOOGLE_CLIENT_ID === void 0 || process.env.GOOGLE_CLIENT_SECRET === void 0) {
    return null;
  } else {
    return google(auth, {
...
const githubAuth = (() => {
  if (process.env.GITHUB_CLIENT_ID === void 0 || process.env.GITHUB_CLIENT_SECRET === void 0) {
    return null;
  } else {
    return github(auth, {
...
class TokenType {
   ...
   */
  async issue(user_id) {
    await prisma$1.singleUseKey.deleteMany({
      where: { user_id, purpose: this.purpose }
    });
    const token = generateRandomString(32);
    await prisma$1.singleUseKey.create({
      data: {
        user_id,
        id: token,
        purpose: this.purpose,
        expires: new Date(Date.now() + this.expiresIn * 1e3)
      }
    });
    return token;
  }
  ...
  async validate(token) {
    const singleUseKey = await prisma$1.singleUseKey.findUnique({
      where: { id: token }
    });
    if (singleUseKey === null || singleUseKey.purpose !== this.purpose || singleUseKey.expires < /* @__PURE__ */ new Date()) {
...
const emailVerificationToken = new TokenType("email-verification", 7 * 24 * 60 * 60);
const resetPasswordToken = new TokenType("reset-password", 10 * 60);
export {
  auth as a,
  googleAuth as b,
  emailVerificationToken as e,
  githubAuth as g,
  resetPasswordToken as r
};
